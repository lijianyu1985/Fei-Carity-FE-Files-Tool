...
export const showFileNameDialog = async (args, type: ResourceType, defaultTypeName): Promise<IPath> => {
  ...
  //           return resourceOptionsTags.includes(option);
  //         })
  //         .map((t) => {
  ...
  return {
    ...
    dirPath: '',
    rootPath: '',
    paramsMap: new Map<OptionType, string>(),
    params: [],
  };
};
...
export const showOptionsDialog = async (config: IConfig, resource: ResourceType): Promise<any> => {
  ...
  const resourceOptionsChoices = [...optionMap.entries()].map((option) => {
    ...
    const optionDefaultValue = optionConfiguredDefaultValue || '';

    const displayValue = (optionDefaultValue && optionDefaultValue !== '') ? `${optionName} (default: ${optionDefaultValue})` : optionName;
    return { label: displayValue, description: optionItem.description } as vscode.QuickPickItem;
  });
  ...
  const selectedOptionTypes = selectedOptions.map(o => o.label.split(' ')[0].trim()).map(l => optionMap.get(l));
...
export const configureOptionsValues = async (config: IConfig, resource: ResourceType, optionTypes: OptionType[]): Promise<Map<OptionType, string>> => {
  ...
  await asyncForEach(optionTypes, async (ot) => {
    ...
    const val = (optionItem.type) ? sortedItems.length == 1 ? 'true' : await vscode.window.showQuickPick(sortedItems, params) : await vscode.window.showInputBox(params);
